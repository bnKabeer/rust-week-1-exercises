...
pub const CURRENT_BLOCK_HEIGHT: u64 = 9000_000;
...
pub fn find_high_fee(fee_list: &[f64]) -> Option<(usize, f64)> {
    ...
    fee_list.iter().enumerate().find(|(_, &fee)| fee > 0.005).map(|(index, &fee)| (index, fee))
}
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    if height < 0 {
        ...
        (false, "Block height is unrealistically high".into())
    } else {
...
pub fn extract_tx_version(raw_tx_hex: &str) -> Result<u32, String> {
    ...
    // Bitcoin uses little-endian byte order for the version field
